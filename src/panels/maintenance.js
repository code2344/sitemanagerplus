...
export function createMaintenancePanel(cluster, watchdog) {
  ...
  // Dedicated maintenance pages
  router.get('/feedback-ui', (req, res) => {
    const adminUiDir = path.join(config.paths.src, 'panels', 'public', 'admin');
    const feedbackFile = path.join(adminUiDir, 'feedback.html');
    if (!fs.existsSync(feedbackFile)) {
      return res.status(404).send('Page not found');
    }
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    return res.send(fs.readFileSync(feedbackFile, 'utf8'));
  });

  /**
   * Feedback endpoints for maintenance (same as /admin)
   */
  
  /**
   * GET /maintenance/feedback/list - List all feedback items
   */
  ...
  /**
   * GET /maintenance/feedback/state - Get feedback enabled state
   */
  ...
  /**
   * POST /maintenance/feedback/toggle - Toggle feedback enabled state (admin only)
   */
  ...
  /**
   * GET /maintenance/feedback/:id - Get a specific feedback item
   */
  ...
  /**
   * POST /maintenance/feedback/submit - Create new feedback item
   */
  ...
  /**
   * POST /maintenance/feedback/:id/comment - Add comment to feedback
   */
  ...
  /**
   * POST /maintenance/feedback/:id/resolve - Resolve feedback (admin only)
   */
  ...
  /**
   * DELETE /maintenance/feedback/:id - Delete feedback (admin only)
   */
